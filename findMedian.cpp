...
int randomizedPartition(int input[], int p, int r) { //             n
...
void randomizedQuickSort(int input[], int p, int r) {//             1+2*T(n/2)
    int q;
    if (p < r) {
...
int partition(int input[], int a, int p, int r) {               //6+4n
    int x = a, i = p - 1;
    for (int j = p; j < r; j++) {
        if (input[j] == x) {
...
void quickSort(int input[], int p, int r) {                     //1+(6+4n) + T(n-1) + T(1)
    int q;
    if (p < r) {
...
int select(int input[], int n, int k, int start, int end, int flag) {
    ...
    if (k == m)         ///O(6)
        ...
        return select(input, m - 1, k, start, q - 1, 0);                //T(n-1)
    else
